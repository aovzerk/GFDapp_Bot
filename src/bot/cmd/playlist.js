...
class Command extends Base_Command {
	...
	run(args) {
		...
		if (guildQueue == undefined || guildQueue == null) {
			...
			queue.join(args.msg.member.voice.channel).then(() => {
				args.msg.channel.send({ "content": `Ищу трек по запросу \`\`${song_request}\`\`` }).then(new_msg => {
					...
					queue.playlist(args.args[1], { "requestedBy": args.msg.member }).then((song) => {
						const embed = new MessageEmbed()
							.setTitle("Добавлен плелист в очередь")
							.setDescription(`[${song.name}](${song.url})`);
						args.msg.channel.send({ "embeds": [embed] }).then(_new_msg => {
		...
		} else if (args.msg.member.voice.channel && args.msg.member.voice.channel.id == args.msg.guild.me.voice.channel.id) {
			guildQueue.playlist(args.args[1], { "requestedBy": args.msg.member }).then((song) => {
				const embed = new MessageEmbed()
					.setTitle("Добавлен плелист в очередь")
					.setDescription(`[${song.name}](${song.url})`);
