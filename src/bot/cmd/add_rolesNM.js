...
const name_command = "add_rolesNM";
...
class Command extends Base_Command {
	...
	run(args) {
		...
		if (args.args[1] == "") {
			args.server_db.set("roles_add_nm", null);
			args.server_db.save();
			embed.setDescription("Выдача ролей новым юзерам выключена");
		...
		let description = "Установлены следующие роли для новых юзеров: ";
		roles.forEach(role_id => {
			description = `${description }<@&${ role_id }>, `;
		});
		const _roles_str = roles.join(",");
		args.server_db.set("roles_add_nm", _roles_str);
		embed.setDescription(description);
		args.msg.reply({ "embeds": [embed] });
		args.server_db.save();
	}
